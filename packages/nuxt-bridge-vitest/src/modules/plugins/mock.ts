...
export const createMockPlugin = (ctx: MockPluginContext) =>
  createUnplugin(() => {
    ...
    function transform(
      ...
      walk(ast as any, {
        enter: (node, parent) => {
          ...
          if (
            ...
            if (
              !isLiteral(importName) ||
              typeof importName.value !== "string"
            ) {
              return this.error(
                new Error(
            ...
            const importItem = ctx.imports.find(
              (_) => name === (_.as || _.name)
            );
            if (!importItem) {
          ...
          if (
            ...
            if (
              !isLiteral(componentName) ||
              typeof componentName.value !== "string"
            ) {
              return this.error(
                new Error(
      ...
      if (mocksImport.length) {
        ...
        mockLines.push(
          ...Array.from(mockImportMap.entries()).flatMap(([from, mocks]) => {
            ...
            const lines = [
              ...
              `    mocks[${JSON.stringify(
                from
              )}] = { ...await importOriginal(${JSON.stringify(from)}) }`,
              `  }`,
            ];
    ...
    return {
      ...
      vite: {
        ...
        async configResolved(config) {
          ...
          if (firstSetupFile) {
            resolvedFirstSetupFile = await resolvePath(
              normalize(resolve(firstSetupFile))
            );
          }
          ...
          const vitestPlugins = plugins.filter(
            (p) =>
              ...
              (p.enforce || (p as any).order) === "post"
          );
          const lastNuxt = findLastIndex(plugins, (i) =>
            i.name?.startsWith("nuxt:")
          );
...
function startOf(node: Node) {
  return "range" in node && node.range
    ? node.range[0]
    : ((node as any).start as number);
}
function endOf(node: Node) {
  return "range" in node && node.range
    ? node.range[1]
    : ((node as any).end as number);
}
