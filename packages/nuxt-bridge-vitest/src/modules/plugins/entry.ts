...
export const NuxtRootStubPlugin = createUnplugin(
  (options: NuxtRootStubPluginOptions) => {
      ...
      vite: {
        async resolveId(id, importer) {
          if (id.endsWith(STUB_ID) || id.endsWith(STUB_ID_WITH_EXT)) {
            return importer?.endsWith("index.html")
              ? id
              : join(dirname(options.entry), STUB_ID_WITH_EXT);
          }
        },
        async load(id) {
          if (id.endsWith(STUB_ID) || id.endsWith(STUB_ID_WITH_EXT)) {
